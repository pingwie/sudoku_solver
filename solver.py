...
class Solver:
    def __init__(self, sudoku):
        self.cube = np.ones((9,9,9), dtype = bool)
    ...
    def least_options_cell(self):
        cell_sums = np.sum(self.cube, axis=2)
        cell_sums[cell_sums == 0] = 10
        min_sum_ind = np.unravel_index(np.argmin(cell_sums), cell_sums.shape)
        return min_sum_ind
    
    def can_place_number(self, i, j, number):
        return self.cube[i][j][number-1]

    def place_number(self, i, j, number):
        self.sudoku[i][j] = number
        self.cube[i][j] = np.zeros(9, dtype=bool)
        for k in range(9):
            self.cube[i][k][number - 1] = False
            self.cube[k][j][number - 1] = False
        for k in range(3):
            for l in range(3):
                self.cube[i//3*3+k][j//3*3+l][number-1] = False
            ...
            for i in range(9):
                for j in range(9):
                    if self.is_trivial_cell(i,j):
                        changed = True
                        self.place_number(i, j, np.argmax(self.cube[i][j]) + 1)

    def is_trivial_cell(self, i, j):
        return self.sudoku[i][j] == 0 and np.sum(self.cube[i][j]) == 1